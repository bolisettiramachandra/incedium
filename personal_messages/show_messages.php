...
if(isset($_GET['message'])){
    ...
    if($obj->Normal_Query("SELECT clean_message_id FROM clean WHERE clean_user_id = ?",[$user_id])){
        $obj->Normal_Query("SELECT  DISTINCT messages.msg_id,messages.message,messages.msg_type,messages.user_id,messages.msg_to,messages.msg_time , (select users.id from users where users.id=messages.user_id) as id_from, (select users.id from users where users.id=messages.msg_to) as id_to, (select users.name from users where users.id=messages.user_id) as username, (select users.image from users where users.id=messages.user_id) as userimage, (select users.status from users where users.id=messages.msg_to) as userstatus
FROM messages,users WHERE messages.user_id=(select users.id from users where users.id=$user_id ) and messages.msg_to=(select users.id from users where users.id=$to_id)
or messages.user_id=(select users.id from users where users.id=$to_id) and messages.msg_to=(select users.id from users where users.id=$user_id) order by messages.msg_id");
